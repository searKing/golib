...
func (thiz *Throwable) InitCause(cause ThrowableInterface) ThrowableInterface {
	if thiz.cause != thiz {
		return NewIllegalStateException2("Can't overwrite cause with "+object.ToString(cause, "a nil"), thiz)
	}
	if cause == thiz {
